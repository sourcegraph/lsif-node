...
export class VertexBuilder {
  ...
  public event(kind: EventKind, scope: Project | Document): Event {
    let result: ProjectEvent | DocumentEvent = {
      ...
      kind,
      scope:
        scope.label === 'project' ? EventScope.project : EventScope.document,
      data: scope.id,
    }
    return result
  }
  ...
  public hoverResult(
    value: lsp.Hover | lsp.MarkupContent | lsp.MarkedString | lsp.MarkedString[]
  ): HoverResult {
  ...
  public item(
    ...
    if (to.length === 0) {
      ...
      if (from.label === 'referenceResult') {
        result.property =
          property !== undefined ? property : ItemEdgeProperties.references
      }
      return result
    }
