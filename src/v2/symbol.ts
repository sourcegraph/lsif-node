...
export class AliasSymbolData extends SymbolData {
  ...
  public begin(): void {
    super.begin()
    this.emitter.emit(
      this.builder.edge.next(this.getResultSet(), this.aliased.getResultSet())
    )
  }
  ...
  public addDefinition(
    ...
    if (this.rename) {
      ...
      this.emitter.emit(this.builder.edge.next(definition, this.getResultSet()))
      super.addReference(sourceFile, definition, ItemEdgeProperties.references)
    }
  }
  ...
  public addReference(
    ...
    if (reference.label === VertexLabels.range) {
      this.emitter.emit(this.builder.edge.next(reference, this.getResultSet()))
    }
...
export class MethodSymbolData extends SymbolData {
  ...
  public addDefinition(
    ...
    for (const base of this.bases) {
      base.addReference(sourceFile, definition, ItemEdgeProperties.definitions)
    }
  }
