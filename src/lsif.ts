...
namespace Converter {
    ...
    export function asDiagnostic(
        ...
        return {
            ...
            code: value.code,
            message: tss.flattenDiagnosticMessageText(
                value.messageText,
                os.EOL
            ),
...
class DocumentData extends LSIFData {
    ...
    public addDocumentSymbols(
        documentSymbols: RangeBasedDocumentSymbol[]
    ): void {
...
class StandardSymbolData extends SymbolData {
    ...
    public findDefinition(
        ...
        if (partition === null) {
            throw new Error(
                `The partition for source file ${sourceFile.fileName}`
            )
        }
    ...
    public getOrCreateDefinitionResult(): DefinitionResult {
        if (this.definitionResult === undefined) {
            ...
            this.emit(
                this.edge.definition(this.resultSet, this.definitionResult)
            )
        }
    ...
    public getOrCreateReferenceResult(): ReferenceResult {
        if (this.referenceResult === undefined) {
            ...
            this.emit(
                this.edge.references(this.resultSet, this.referenceResult)
            )
        }
    ...
    public getOrCreateTypeDefintionResult(): TypeDefinitionResult {
        if (this.typeDefinitionResult === undefined) {
            ...
            this.emit(
                this.edge.typeDefinition(
                    this.resultSet,
                    this.typeDefinitionResult
                )
            )
        }
...
class MethodSymbolData extends StandardSymbolData {
    ...
    public addDefinition(
        ...
        if (this.bases !== undefined) {
            for (let base of this.bases) {
                base.getOrCreatePartition(sourceFile).addReference(
                    definition,
                    ItemEdgeProperties.definitions
                )
            }
...
class MethodResolver extends SymbolDataResolver {
    ...
    public resolve(
        ...
        let baseMembers = this.symbols.findBaseMembers(
            container,
            symbol.getName()
        )
...
class UnionOrIntersectionResolver extends SymbolDataResolver {
    ...
    public getDeclarationNodes(
        ...
        if (location === undefined) {
            throw new Error(
                `Union or intersection resolver requires a location`
            )
        }
        return [location]
    }
    ...
    public forwardSymbolInformation(symbol: ts.Symbol): void {
        this.visitSymbol(
            ...
            (index: number, typeAlias: ts.Symbol, literalType: ts.Symbol) => {
                ...
                this.symbols.storeSymbolAlias(literalType, {
                    alias: typeAlias,
                    name,
                })
                return index
...
export class DataManager implements SymbolDataContext {
    ...
    public projectProcessed(): void {
        ...
        console.log(
            `${this.documentStats} file(s), ${this.symbolStats} symbol(s)`
        )
...
class Visitor implements ResolverContext {
                ...
                if (node.exportClause !== undefined) {
                    ...
                    for (let { name, propertyName } of elements) {
                        ...
                        if (propertyName !== undefined) {
                            ...
                            localSymbol = this.typeChecker.getAliasedSymbol(
                                exportSymbol
                            )
                        }
    ...
    private addDocumentSymbol(node: tss.Declaration): boolean {
        ...
        let symbol = this.program
            .getTypeChecker()
            .getSymbolAtLocation(rangeNode)
        let declarations =
        ...
        let currentContainer = this.symbolContainer[
            this.symbolContainer.length - 1
        ]
    ...
    public getOrCreateDocumentData(sourceFile: ts.SourceFile): DocumentData {
        ...
        let document = this.vertex.document(
            sourceFile.fileName,
            sourceFile.text
        )
    ...
    public getOrCreateSymbolData(
        ...
        const declarations:
            | ts.Node[]
            | undefined = resolver.getDeclarationNodes(symbol, location)
        ...
        if (tss.isSourceFile(symbol) && monikerPath !== undefined) {
            monikerIdentifer = tss.createMonikerIdentifier(
                monikerPath,
                undefined
            )
        ...
        for (let declaration of declarations) {
            ...
            const [
                identifierNode,
                identifierText,
            ] = resolver.getIdentifierInformation(
                sourceFile,
                symbol,
                declaration
            )
            if (identifierNode !== undefined && identifierText !== undefined) {
                ...
                if (
                    hover === undefined &&
                    tss.isNamedDeclaration(declaration)
                ) {
                    // let start = Date.now();
    ...
    private resolveEmittingNode(
        ...
        if (tss.isAliasSymbol(symbol)) {
            ...
            return this.isFullContentIgnored(sourceFile)
                ? undefined
                : sourceFile
        }
        if (ts.isSourceFile(declaration)) {
            return this.isFullContentIgnored(declaration)
                ? undefined
                : declaration
        }
